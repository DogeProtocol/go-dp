...
func TestSimulatedBackend(t *testing.T) {
	...
	auth, _ := bind.NewKeyedTransactorWithChainID(key, big.NewInt(types.DEFAULT_CHAIN_ID))
	...
	tx, _ = types.SignTx(tx, types.NewLondonSignerDefaultChain(), key)
...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != params.AllProofOfStakeProtocolChanges {
		t.Errorf("expected sim config to equal params.AllProofOfStakeProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != params.AllProofOfStakeProtocolChanges {
		t.Errorf("expected sim blockchain config to equal params.AllProofOfStakeProtocolChanges, got %v", sim.config)
	}
...
func TestNewAdjustTimeFail(t *testing.T) {
	...
	signedTx, err := types.SignTx(tx, types.NewLondonSignerDefaultChain(), testKey)
	if err != nil {
	...
	signedTx2, err := types.SignTx(tx2, types.NewLondonSignerDefaultChain(), testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestEstimateGas(t *testing.T) {
	...
	opts, _ := bind.NewKeyedTransactorWithChainID(key, big.NewInt(types.DEFAULT_CHAIN_ID))
	...
	var cases = []struct {
		...
		{"plain transfer(valid)", dp.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", dp.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestEstimateGasWithPrice(t *testing.T) {
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", dp.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithVeryHighPrice", dp.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", dp.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
...
func TestPendingCodeAt(t *testing.T) {
	...
	auth, _ := bind.NewKeyedTransactorWithChainID(testKey, big.NewInt(types.DEFAULT_CHAIN_ID))
...
func TestPendingAndCallContract(t *testing.T) {
	...
	contractAuth, _ := bind.NewKeyedTransactorWithChainID(testKey, big.NewInt(types.DEFAULT_CHAIN_ID))
	...
	res, err := sim.PendingCallContract(bgCtx, dp.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, dp.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestCallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, dp.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, dp.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
	...
	for _, cl := range call {
		for key, val := range inputs {
			...
			if val != nil {
				...
				if !ok {
					t.Fatalf("expect revert error")
				}
				if rerr == nil {
					t.Fatalf("rerr is nil")
				}
...
func TestForkResendTx(t *testing.T) {
	...
	tx, _ := types.SignTx(_tx, types.NewLondonSignerDefaultChain(), testKey)
