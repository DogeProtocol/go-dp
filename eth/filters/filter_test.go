...
func BenchmarkFilters(b *testing.B) {
	...
	chain, receipts := core.GenerateChain(params.TestChainConfig, genesis, mockconsensus.NewMockConsensus(), db, 100010, func(i int, gen *core.BlockGen) {
		switch i {
		case 2403:
...
func TestFilters(t *testing.T) {
	...
	chain, receipts := core.GenerateChain(params.TestChainConfig, genesis, mockconsensus.NewMockConsensus(), db, 1000, func(i int, gen *core.BlockGen) {
		switch i {
		case 1:
			receipt := types.NewReceipt(nil, false, 0)
			receipt.Logs = []*types.Log{
			...
			gen.AddUncheckedReceipt(receipt)
			gen.AddUncheckedTx(types.NewTransaction(1, common.HexToAddress("0x1"), big.NewInt(1), 1, nil, nil))
		case 2:
			receipt := types.NewReceipt(nil, false, 0)
			receipt.Logs = []*types.Log{
				{
			...
			gen.AddUncheckedReceipt(receipt)
			gen.AddUncheckedTx(types.NewTransaction(2, common.HexToAddress("0x2"), big.NewInt(2), 2, nil, nil))
		...
		case 998:
			...
			gen.AddUncheckedReceipt(receipt)
			gen.AddUncheckedTx(types.NewTransaction(998, common.HexToAddress("0x998"), big.NewInt(998), 998, nil, nil))
		case 999:
			receipt := types.NewReceipt(nil, false, 0)
			receipt.Logs = []*types.Log{
				{
