...
func testHybridedsSigBasic(t *testing.T, NativeGolangVerify bool) {
	var sig signaturealgorithm.SignatureAlgorithm

	if NativeGolangVerify {
		sig = CreateHybridedsSig(true)
	} else {
		sig = CreateHybridedsSig(false)
	}
	signaturealgorithm.SignatureAlgorithmTest(t, sig)
}

func TestHybridedsSig_Basic(t *testing.T) {
	fmt.Println("NativeGolangVerify false test")
	testHybridedsSigBasic(t, false)
