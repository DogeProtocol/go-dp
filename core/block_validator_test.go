...
func TestBlockUnhashedFields(t *testing.T) {
	...
	var (
		...
		blocks, _ = GenerateChain(params.TestChainConfig, genesis, mockconsensus.NewMockConsensus(), testdb, 8, nil)
	)
...
func TestHeaderVerification(t *testing.T) {
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, mockconsensus.NewMockConsensus(), vm.Config{}, nil, nil)
	defer chain.Stop()
	...
	for i := 0; i < len(blocks); i++ {
		for j, valid := range []bool{true, false} {
			...
			if valid {
				...
				engine := mockconsensus.NewFailerMockConsensus(headers[i].Number.Uint64())
				_, results = engine.VerifyHeaders(chain, []*types.Header{headers[i]}, []bool{true})
			}
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, mockconsensus.NewFailerMockConsensus(uint64(len(headers)-1)), vm.Config{}, nil, nil)
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	var (
		...
		gspec     = &Genesis{Config: params.TestChainConfig}
		genesis   = gspec.MustCommit(testdb)
		blocks, _ = GenerateChain(params.TestChainConfig, genesis, mockconsensus.NewMockConsensus(), testdb, 1024, nil)
	)
	headers := make([]*types.Header, len(blocks))
	seals := make([]bool, len(blocks))
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, mockconsensus.NewDelayerMockConsensus(time.Millisecond), vm.Config{}, nil, nil)
	defer chain.Stop()
