...
func transaction(nonce uint64, gaslimit uint64, key *oqs.PrivateKey) *types.Transaction {
	return pricedTransaction(nonce, gaslimit, big.NewInt(1), key)
}

func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *oqs.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.HomesteadSigner{}, key)
	return tx
}

func pricedDataTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *oqs.PrivateKey, bytes uint64) *types.Transaction {
...
func dynamicFeeTx(nonce uint64, gaslimit uint64, gasFee *big.Int, tip *big.Int, key *oqs.PrivateKey) *types.Transaction {
...
func setupTxPool() (*TxPool, *oqs.PrivateKey) {
	return setupTxPoolWithConfig(params.TestChainConfig)
}

func setupTxPoolWithConfig(config *params.ChainConfig) (*TxPool, *oqs.PrivateKey) {
...
func TestTransactionDropping(t *testing.T) {
	...
	account := cryptopq.PubkeyToAddressNoError(key.PublicKey)
...
func TestTransactionPostponing(t *testing.T) {
	...
	keys := make([]*oqs.PrivateKey, 2)
	accs := make([]common.Address, len(keys))

	for i := 0; i < len(keys); i++ {
		keys[i], _ = cryptopq.GenerateKey()
		accs[i] = cryptopq.PubkeyToAddressNoError(keys[i].PublicKey)

		testAddBalance(pool, cryptopq.PubkeyToAddressNoError(keys[i].PublicKey), big.NewInt(50100))
	}
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	for i := 0; i < len(keys); i++ {
		keys[i], _ = cryptopq.GenerateKey()
		testAddBalance(pool, cryptopq.PubkeyToAddressNoError(keys[i].PublicKey), big.NewInt(1000000))
	}
	...
	if nolocals {
		...
		if queued := pool.queue[cryptopq.PubkeyToAddressNoError(local.PublicKey)].Len(); uint64(queued) != 3*config.GlobalQueue {
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	local, _ := cryptopq.GenerateKey()
	remote, _ := cryptopq.GenerateKey()

	testAddBalance(pool, cryptopq.PubkeyToAddressNoError(local.PublicKey), big.NewInt(1000000000))
	testAddBalance(pool, cryptopq.PubkeyToAddressNoError(remote.PublicKey), big.NewInt(1000000000))
	...
	statedb.SetNonce(cryptopq.PubkeyToAddressNoError(remote.PublicKey), 2)
	statedb.SetNonce(cryptopq.PubkeyToAddressNoError(local.PublicKey), 2)
	<-pool.requestReset(nil, nil)
...
func TestTransactionPoolRepricingKeepsLocals(t *testing.T) {
	...
	// Create a number of test accounts and fund them
	keys := make([]*oqs.PrivateKey, 3)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = cryptopq.GenerateKey()
		testAddBalance(pool, cryptopq.PubkeyToAddressNoError(keys[i].PublicKey), big.NewInt(1000*1000000))
	}
...
func TestDualHeapEviction(t *testing.T) {
	...
	add := func(urgent bool) {
		...
		for i := range txs {
			...
			key, _ := cryptopq.GenerateKey()
			testAddBalance(pool, cryptopq.PubkeyToAddressNoError(key.PublicKey), big.NewInt(1000000000000))
			if urgent {
...
func TestTransactionDeduplication(t *testing.T) {
	...
	key, _ := cryptopq.GenerateKey()
	testAddBalance(pool, cryptopq.PubkeyToAddressNoError(key.PublicKey), big.NewInt(1000000000))
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	pool.Stop()
	statedb.SetNonce(cryptopq.PubkeyToAddressNoError(local.PublicKey), 1)
	blockchain = &testBlockChain{statedb, 1000000, new(event.Feed)}
...
func BenchmarkInsertRemoteWithAllLocals(b *testing.B) {
	...
	remoteKey, _ := cryptopq.GenerateKey()
	remoteAddr := cryptopq.PubkeyToAddressNoError(remoteKey.PublicKey)
