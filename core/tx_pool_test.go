...
func dynamicFeeTx(nonce uint64, gaslimit uint64, gasFee *big.Int, tip *big.Int, key *signaturealgorithm.PrivateKey) *types.Transaction {
...
func TestDualHeapEviction(t *testing.T) {
	...
	add := func(urgent bool) {
		...
		for i := range txs {
			...
			key, _ := cryptobase.SigAlg.GenerateKey()
			testAddBalance(pool, cryptobase.SigAlg.PublicKeyToAddressNoError(&key.PublicKey), big.NewInt(1000000000000))
			if urgent {
