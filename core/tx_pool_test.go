...
func transaction(nonce uint64, gaslimit uint64, key *signaturealgorithm.PrivateKey) *types.Transaction {
	return pricedTransaction(nonce, gaslimit, big.NewInt(1), key)
}

func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *signaturealgorithm.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.NewLondonSignerDefaultChain(), key)
	return tx
}

func pricedDataTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *signaturealgorithm.PrivateKey, bytes uint64) *types.Transaction {
	data := make([]byte, bytes)
	rand.Read(data)

	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(0), gaslimit, gasprice, data), types.NewLondonSignerDefaultChain(), key)
	return tx
}

func setupTxPool() (*TxPool, *signaturealgorithm.PrivateKey) {
	return setupTxPoolWithConfig(params.TestChainConfig)
}

func setupTxPoolWithConfig(config *params.ChainConfig) (*TxPool, *signaturealgorithm.PrivateKey) {
...
func deriveSender(tx *types.Transaction) (common.Address, error) {
	return types.Sender(types.NewLondonSignerDefaultChain(), tx)
}
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil), types.NewLondonSignerDefaultChain(), key)
	from, _ := deriveSender(tx)
	testAddBalance(pool, from, big.NewInt(1))
...
func TestTransactionDropping(t *testing.T) {
	...
	account := cryptobase.SigAlg.PublicKeyToAddressNoError(&key.PublicKey)
...
func TestTransactionPostponing(t *testing.T) {
	...
	keys := make([]*signaturealgorithm.PrivateKey, 2)
	accs := make([]common.Address, len(keys))

	for i := 0; i < len(keys); i++ {
		keys[i], _ = cryptobase.SigAlg.GenerateKey()
		accs[i] = cryptobase.SigAlg.PublicKeyToAddressNoError(&keys[i].PublicKey)

		testAddBalance(pool, cryptobase.SigAlg.PublicKeyToAddressNoError(&keys[i].PublicKey), big.NewInt(50100))
	}
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	for i := 0; i < len(keys); i++ {
		keys[i], _ = cryptobase.SigAlg.GenerateKey()
		testAddBalance(pool, cryptobase.SigAlg.PublicKeyToAddressNoError(&keys[i].PublicKey), big.NewInt(1000000))
	}
	...
	if nolocals {
		...
		if queued := pool.queue[cryptobase.SigAlg.PublicKeyToAddressNoError(&local.PublicKey)].Len(); uint64(queued) != 3*config.GlobalQueue {
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	local, _ := cryptobase.SigAlg.GenerateKey()
	remote, _ := cryptobase.SigAlg.GenerateKey()

	testAddBalance(pool, cryptobase.SigAlg.PublicKeyToAddressNoError(&local.PublicKey), big.NewInt(1000000000))
	testAddBalance(pool, cryptobase.SigAlg.PublicKeyToAddressNoError(&remote.PublicKey), big.NewInt(1000000000))
	...
	statedb.SetNonce(cryptobase.SigAlg.PublicKeyToAddressNoError(&remote.PublicKey), 2)
	statedb.SetNonce(cryptobase.SigAlg.PublicKeyToAddressNoError(&local.PublicKey), 2)
	<-pool.requestReset(nil, nil)
...
func TestTransactionPoolRepricingKeepsLocals(t *testing.T) {
	...
	// Create a number of test accounts and fund them
	keys := make([]*signaturealgorithm.PrivateKey, 3)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = cryptobase.SigAlg.GenerateKey()
		testAddBalance(pool, cryptobase.SigAlg.PublicKeyToAddressNoError(&keys[i].PublicKey), big.NewInt(1000*1000000))
	}
...
func TestTransactionDeduplication(t *testing.T) {
	...
	key, _ := cryptobase.SigAlg.GenerateKey()
	testAddBalance(pool, cryptobase.SigAlg.PublicKeyToAddressNoError(&key.PublicKey), big.NewInt(1000000000))
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	pool.Stop()
	statedb.SetNonce(cryptobase.SigAlg.PublicKeyToAddressNoError(&local.PublicKey), 1)
	blockchain = &testBlockChain{statedb, 1000000, new(event.Feed)}
...
func BenchmarkInsertRemoteWithAllLocals(b *testing.B) {
	...
	remoteKey, _ := cryptobase.SigAlg.GenerateKey()
	remoteAddr := cryptobase.SigAlg.PublicKeyToAddressNoError(&remoteKey.PublicKey)
